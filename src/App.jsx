  ...
  function filterIds() {
    ...
    filterIdList.map((currentFilter) => {
      ...
      if (currentFilter === filterState) {
        ...
        document.getElementById(currentFilter).style.backgroundColor =
          "transparent";
    ...
    emailListStore.map((currentEmail) => {
      if (currentEmail.id === clickedId) {
        ...
        document.getElementById(
          "email-description"
        ).innerHTML += `<p>${loremIpsumPara1}</p>`;
        document.getElementById(
          "email-description"
        ).innerHTML += `<p>${loremIpsumPara2}</p>`;
        document.getElementById(
          "email-description"
        ).innerHTML += `<p>${loremIpsumPara3}</p>`;
        console.log(document.getElementById("email-description").innerText);

        document.querySelector(".email-body-container1").innerText =
          currentEmail.from.name[0];

        document.querySelector(".emailBody-subject").innerText =
  ...
  useEffect(() => {
    async function getEmailList() {
      try {
        ...
        if (emailListStore.length !== 0) return;
              ...
              emailListStore.map((emailData) => {
                if (emailData.is_read == true) {
                  return (
                    <div
                      key={emailData.id}
                      className="emailData-container read-color"
                    >
                      ...
                      <a
                        href="#"
                        className="emailData-container2"
                        onClick={() => {
                          setClickedId(`${emailData.id}`);
                          setCurrentEmailDisplayed(emailData);
                          emailData.is_read = true;
                          markIsReadTrue({ email: emailData });
        ...
        >
          ...
          <div className="email-body-container2">
            ...
            <div className="emailBody-header">
              <b>
                <span className="emailBody-subject"></span>
              </b>
              <button
                className="fav"
                onClick={() => {
                  currentEmailDisplayed.is_favorite = true;
                  markIsFavoriteTrue({ email: currentEmailDisplayed });
